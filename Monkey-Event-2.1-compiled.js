var MonkeyEventInit = function () {
	...
	previousDist,
		...
		if (typeof currentEvent.up === "function") currentEvent.up({
			distanceX: mvalx,
			distanceY: mvaly,
			speedX: _speed.x,
			speedY: _speed.y,
		...
		if (Math.abs(mvalx) >= Math.abs(mvaly)) {
			if (mvalx >= _cz) {
				if (typeof currentEvent.right === "function") currentEvent.right(mvalx);
			} else if (mvalx <= -_cz) {
				if (typeof currentEvent.left === "function") currentEvent.left(mvalx);
			}
		} else {
			if (mvaly >= _cz) {
				if (typeof currentEvent.bottom === "function") currentEvent.bottom(mvaly);
			} else if (mvaly <= -_cz) {
				if (typeof currentEvent.top === "function") currentEvent.top(mvaly);
			}
		}
		...
		if (!ISPC) {
			...
			if (e.targetTouches.length > 1) {
				...
				if (previousAngle && typeof currentEvent.rotate === 'function') {
					...
					if (_speed > 2.5) _speed = -(1.57 - _angle + 1.57 + previousAngle);else if (_speed < -2.5) _speed = 1.57 - previousAngle + 1.57 + _angle;
		...
		if (typeof currentEvent.move === "function") currentEvent.move({
	...
	function EventFactory(o) {
		...
		this.MOUSE_DOWN = function (e) {
			...
			if (typeof _s.down === "function") _s.down(e);
	...
	function MonkeyEventInit(o) {
		for (var i = 0; i < EvList.length; i++) {
			if (EvList[i].e == o) return EvList[i];
		}
...
//# sourceMappingURL=Monkey-Event-2.1-compiled.js.map